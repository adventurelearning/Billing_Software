...
const CalculatorPopup = ({ onClose }) => {
  ...
  const handleSpecialOperation = (op) => {
    ...
    switch (op) {
      ...
      case "square":
        ...
        break;
      case "inverse":
        result = current === 0 ? "Error" : 1 / current;
        break;
      case "negate":
        result = current * -1;
        break;
  ...
  return (
    ...
    >
      <div
        className={`rounded-2xl shadow-2xl w-96 overflow-hidden ${
      ...
      >
        ...
        >
          ...
          <button onClick={onClose} className="p-2 rounded-full text-sm">
            ✕
          </button>
        </div>
        ...
        <div className={`p-4 ${darkMode ? "bg-gray-700" : "bg-gray-50"}`}>
          {showHistory && (
            <div
              className={`mb-2 p-2 rounded text-sm h-20 overflow-y-auto ${
                darkMode ? "bg-gray-600" : "bg-gray-200"
              }`}
            >
              {history.map((item, index) => (
                <div key={index} className="mb-1 opacity-70">
                  {item}
                </div>
              ))}
            </div>
